...
void WfmBase::SetTrackInDataNames(TString DataDir, TString DataName)
{
    ...
    if (DataName != "")
       {
        if (DataName.Contains(".root")) m_Trackdataname.push_back(DataName(0, DataName.Length() - 5));
          ...
          if (m_Trackdataname.size() == 0)
             {
              std::cout << __FUNCTION__ << " INFO: Scanning for tracking files in folder " << m_Trackdatadir << " ... ";
...
// --------------------------------------------------------------------------------------------------------------
bool WfmBase::OpenTrackFile(TString trackdirdir, std::vector<TString> trackdataname, std::vector<unsigned int>& filesIndx)
{
    ...
    for (unsigned int i = 0; i < trackdataname.size(); i++)
        {
         ...
         found = false;
         for (unsigned int as = 0; as < filesIndx.size(); as++) { if (i == filesIndx.at(as)) { found = true; binaryindx = as; break; } }
         if (!found) continue;
         m_trackfiles.push_back(TFile::Open(trackdirdir+trackdataname.at(i)+".root", "READ"));
         std::cout << "    " << __FUNCTION__ << " INFO: Opening tracking file " << (TString)(trackdirdir + trackdataname.at(i) + ".root") << "..." << std::endl;
         files++;
         if (m_trackfiles.at(files-1) && !(m_trackfiles.at(files-1)->IsZombie()))
            { 
             ...
             if ((TTree*)(m_trackfiles.at(files-1))->Get("TreeWriterPassive/USBPIX_GEN2_BOARD_131_20/tracks"))
                {     
                 ...
                 if (WfmBase::GetVerbosity() >= 1) std::cout << "    " << __FUNCTION__ << " INFO: Scanned file " << i + 1 << "/" << trackdataname.size() << ", found " << m_nDUT.back()
                                                             << " DUTs, " << m_nRef.back() << " references and " << m_nplanes.back() << " telescope planes" << std::endl;
                }
    ...
    if (GetTrackPackage() == "Cory")
       {
        // ROI Pointers
        b_m_ROI.clear();
        b_m_ROI.resize(11*files, nullptr);
        m_ROI_Dbl.clear();
        m_ROI_Dbl.resize(8*files, nullptr);
        m_ROI_Int.clear();
        m_ROI_Int.resize(3*files, nullptr);
        NofPlanes = std::accumulate(m_nplanes.begin(), m_nplanes.end(), 0);
        NofRefs = std::accumulate(m_nRef.begin(), m_nRef.end(), 0);
        NofDUTs = std::accumulate(m_nDUT.begin(), m_nDUT.end(), 0);
        if (WfmBase::GetVerbosity() >= 1) std::cout << "    " << __FUNCTION__ << " INFO: Total number of planes: " << NofPlanes 
                                                    << ", of references: " << NofRefs << ", and of DUTs: " << NofDUTs << std::endl;
        // Reference pointers
        b_m_Ref.clear();
        b_m_Ref.resize(5*NofRefs, nullptr);
        m_Ref_Dbl.clear();
        m_Ref_Dbl.resize(4*NofRefs, nullptr);
        m_Ref_Int.clear();
        m_Ref_Int.resize(NofRefs, nullptr);
        // Telescope plane pointers
        b_m_TelPlane.clear();
        b_m_TelPlane.resize(8*NofPlanes, nullptr);
        m_TelPlane_Dbl.clear();
        m_TelPlane_Dbl.resize(6*NofPlanes, nullptr);
        m_TelPlane_Int.clear();
        m_TelPlane_Int.resize(2*NofPlanes, nullptr);
        // DUTs pointers
        b_m_DUT.clear();
        b_m_DUT.resize(5*NofDUTs, nullptr);
        m_DUT_Dbl.clear();
        m_DUT_Dbl.resize(4*NofDUTs, nullptr);
        m_DUT_Int.clear();
        m_DUT_Int.resize(NofDUTs, nullptr);
       }
    else if (GetTrackPackage() == "EUTelescope")
            {
             // Initialize pointer vectors
             b_m_Tracks.clear();
             b_m_Tracks.resize(13*files, nullptr);   //!
             m_TracksInt.clear();
             m_TracksInt.resize(3*files, nullptr);
             m_TracksDbl.clear();
             m_TracksDbl.resize(7*files, nullptr);
             m_TrsInt.clear();
             m_TrsInt.resize(3*files, 0);
             b_m_Hits.clear();
             b_m_Hits.resize(4*files, nullptr);   //!
             m_HitsInt.clear();
             m_HitsInt.resize(4*files, nullptr);
            }
    else if (GetTrackPackage() == "TBA")
            {
               //
         ...
         if (GetTrackPackage() == "Cory")
            {
             ...
             if (WfmBase::GetVerbosity() >= 1) PrintBranches(m_trackTrees.at(treeindx), "ROI");
             // Get Telescope planes trees
             for (unsigned int k = 0; k < m_nplanes.at(f); k++)
                 {
                  ...
                  if (WfmBase::GetVerbosity() >= 1) PrintBranches(m_trackTrees.at(treeindx), Form("TelescopePlane_%01u", k));
                 }
             // Reference trees
             m_nRef.push_back(0);
             for (unsigned int k = 0; k < m_nRef.at(f); k++)
                 {
                  ...
                  if (WfmBase::GetVerbosity() >= 1) PrintBranches(m_trackTrees.at(treeindx), Form("Reference_%01u", k));
                 }
             // Get DUT trees
             m_nDUT.push_back(0);
             for (unsigned int k = 0; k < m_nDUT.at(f); k++)
                 {
                  ...
                  if (WfmBase::GetVerbosity() >= 1) PrintBranches(m_trackTrees.at(treeindx), Form("DUT_%01u", k));
                 }
            }
         else if (GetTrackPackage() == "EUTelescope")
                 {
                  ...
                  if (WfmBase::GetVerbosity() >= 1) PrintBranches(m_trackTrees.at(treeindx), "Tracks");
                 }
         else if (GetTrackPackage() == "TBA")
                 {
                   //
    ...
    // Create dummy pointers to assigne values for non-present planes and events
    m_NoPlaneDbl = new std::vector<double>();
    m_NoPlaneInt = new std::vector<int>();
    m_NoEvntDbl = new std::vector<double>();
    m_NoEvntInt = new std::vector<int>();
    m_NoPlaneDbl->push_back(-100);
    m_NoPlaneInt->push_back(-100);
    m_NoEvntDbl->push_back(-99);
    m_NoEvntInt->push_back(-99);

    if (WfmBase::GetVerbosity() >= 1) std::cout << std::endl;
    if (m_trackTrees.size() == 0) return false;
    else return true;
}
// --------------------------------------------------------------------------------------------------------------
void WfmBase::InitCoryPointers(unsigned int nRef, unsigned int nDUT, unsigned int nplanes)
{
...
// --------------------------------------------------------------------------------------------------------------
void WfmBase::InitEUTelPointers()
{
    // Pointer Initialization
    m_xTr = NULL;
    m_yTr = NULL;
...
// --------------------------------------------------------------------------------------------------------------
bool WfmBase::AddTrckBranches(TTree* c_tree, std::string trckpkg)
{
...
// --------------------------------------------------------------------------------------------------------------
bool WfmBase::CombineTrackCory(int EvntNmbr, std::vector<unsigned int> &evntlast, unsigned int file)
{
    if (EvntNmbr < 0)
       {
    ...
    // Vector pointer cleanup from previous event, one cannot destroy and reacreate the pointers as the InitCory pointers function does
    // because the assigned  branches will be lost. Vectors of pointers need to be maintained as individual allements set to default values
    m_ClusterSizeX_ROI = m_NoEvntDbl;
    m_ClusterSizeY_ROI = m_NoEvntDbl;
    m_PixelX_ROI = m_NoEvntDbl;
    m_PixelY_ROI = m_NoEvntDbl;
    m_InterceptX_ROI = m_NoEvntDbl;
    m_InterceptY_ROI = m_NoEvntDbl;
    m_InterceptResX_ROI = m_NoEvntDbl;
    m_InterceptResY_ROI = m_NoEvntDbl;
    m_HasAsso = m_NoEvntInt;
    m_ClusterNumPixels_ROI = m_NoEvntInt;
    for (unsigned int k = 0; k < m_ntp_planes.size(); k++)
        {
         (m_InterceptX_TelPlane.at(k)) = m_NoEvntDbl;
         (m_InterceptY_TelPlane.at(k)) = m_NoEvntDbl;
         (m_InterceptResX_TelPlane.at(k)) = m_NoEvntDbl;
         (m_InterceptResY_TelPlane.at(k)) = m_NoEvntDbl;
         (m_ClusterSizeX_TelPlane.at(k)) = m_NoEvntDbl;
         (m_ClusterSizeY_TelPlane.at(k)) = m_NoEvntDbl;
         (m_ClusterNumPixels_TelPlane.at(k)) = m_NoEvntInt;
        }
    for (unsigned int k = 0; k < m_ntp_Ref.size(); k++)
        {
         (m_InterceptX_Ref.at(k)) = m_NoEvntDbl;
         (m_InterceptY_Ref.at(k)) = m_NoEvntDbl;
         (m_InterceptResX_Ref.at(k)) = m_NoEvntDbl;
         (m_InterceptResY_Ref.at(k)) = m_NoEvntDbl;
        }
    for (unsigned int k = 0; k < m_ntp_DUT.size(); k++)
        {
         (m_InterceptX_DUT.at(k)) = m_NoEvntDbl;
         (m_InterceptY_DUT.at(k)) = m_NoEvntDbl;
         (m_InterceptResX_DUT.at(k)) = m_NoEvntDbl;
         (m_InterceptResY_DUT.at(k)) = m_NoEvntDbl;
        }
    ...
    for (unsigned int y = 0; y < m_nplanes.at(file); y++)
        {
         ...
         if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: Finished planes tree " << tmptreeindx << std::endl;
        }
    // Step 4: If the plane was not present in this file, populate the vectors only with one entry of -100
    for (unsigned int h = 0; h < control.size(); h++)
        {
         (m_InterceptX_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptY_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResX_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResY_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_ClusterSizeX_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_ClusterSizeY_TelPlane.at(control.at(h))) = m_NoPlaneDbl;
         (m_ClusterNumPixels_TelPlane.at(control.at(h))) = m_NoPlaneInt;
        }
    if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: We finished looking at all telescope plane trees" << std::endl;
              ...
              if ((m_Ref_Int.at(refprfx+y))->at(0) == EvntNmbr)
                 {
                  ...
                  evntlast.at(m_nplanes.at(file)+1+y) = k;
                  break;
                 }
              else continue;
             }
         if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: Finished reference tree " << tmptreeindx << std::endl;
        }
    // Step 4: If the reference was not present in this file, populate the vectors only with one entry of -100
    for (unsigned int h = 0; h < control.size(); h++)
        {
         (m_InterceptX_Ref.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptY_Ref.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResX_Ref.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResY_Ref.at(control.at(h))) = m_NoPlaneDbl;
        }
    if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: We finished looking at all reference plane trees" << std::endl;
              ...
              if ((m_DUT_Int.at(dutprfx+y))->at(0) == EvntNmbr)
                 {
                  ...
                  evntlast.at(m_nplanes.at(file)+1+m_nRef.at(file)+y) = k;
                  break;
                 }
              else continue;
             }
         if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: Finished DUT tree " << tmptreeindx << std::endl;
        }
    // Step 4: If the DUT was not present in this file, populate the vectors only with one entry of -100
    for (unsigned int h = 0; h < control.size(); h++)
        {
         (m_InterceptX_DUT.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptY_DUT.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResX_DUT.at(control.at(h))) = m_NoPlaneDbl;
         (m_InterceptResY_DUT.at(control.at(h))) = m_NoPlaneDbl;
        }
    if (WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " INFO: We finished looking at all DUT trees" << std::endl;

   return true;
}
// --------------------------------------------------------------------------------------------------------------
bool WfmBase::CombineTrackEUTel(int EvntNmbr, std::vector<unsigned int>& evntlast, unsigned int file)
{
...
// --------------------------------------------------------------------------------------------------------------
bool WfmBase::CombineTrackTBA(int EvntNmbr, std::vector<unsigned int>& evntlast, unsigned int file)
{
    //
