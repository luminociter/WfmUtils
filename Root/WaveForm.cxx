...
// --------------------------------------------------------------------------------------------------------------
WaveForm::WaveForm(WfmBase *Base)
{
    m_fitchi2 = -99;
    m_WvBase = Base;
    InitializeWaveForm();
...
// --------------------------------------------------------------------------------------------------------------
// Constructor from WfmBase with time and voltage input
WaveForm::WaveForm(std::vector<double>* voltage, std::vector<double>* time, WfmBase *Base)
{
    m_fitchi2 = -99;
    m_WvBase = Base;
    InitializeWaveForm();
...
// --------------------------------------------------------------------------------------------------------------
// Constructor from WfmBase with rate and voltage input (time vector is computed internaly)
WaveForm::WaveForm(std::vector<double>* voltage, Long64_t snrate, WfmBase *Base)
{
    m_fitchi2 = -99;
    m_WvBase = Base;
    InitializeWaveForm();
...
// --------------------------------------------------------------------------------------------------------------
// Constructor from WfmBase with time, voltage and sampling rate input
WaveForm::WaveForm(std::vector<double>* voltage, std::vector<double>* time, Long64_t snrate, WfmBase *Base)
{
    m_fitchi2 = -99;
    m_WvBase = Base;
    InitializeWaveForm();
...
void WaveForm::InitializeWaveForm(int level)
{
     ...
     if (m_fitchi2 != -99 && m_fitchi2 != -1) delete m_noiseFit;
     m_noiseFit = NULL;
     // Fitting methode comparison arrays used for comparative plots
     m_FitComp = { -99., -99., -99., -99., -99., -99. }; // to initialization
     for (unsigned int k = 0; k < 5; k++) { for (unsigned int h = 0; h < 6; h++) { for (unsigned int g = 0; g < 7; g++) m_FitCompEvnt[k][h][g] = -99.; } }
...
bool WaveForm::Calculate()
{
    ...
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << std::endl << __FUNCTION__ << " INFO: Polarity: " << m_pol << std::endl;
    m_maxIndx = VoltMaxIndx(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Max Voltage Index: " << m_maxIndx << std::endl;
    m_maxVolt = VoltMax(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Max Voltage: " << m_maxVolt << std::endl;
    m_minIndx = VoltMinIndx(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Min Voltage Index: " << m_minIndx << std::endl;
    m_minVolt = VoltMin(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Min Voltage: " << m_minVolt << std::endl;
    m_halfPtsIndx = FindHalfPoints(m_voltage, false);
    if (m_halfPtsIndx.size() < 1)
       {
        if (m_WvBase->WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " WARNING: Could not determine half points, stopping..." << std::endl;
        return false;
       }
    m_IsSignal = IsSignal(&m_halfPtsIndx);   
    if (!m_IsSignal)
       {
        if (m_WvBase->WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__<< " WARNING: Found several reocuring signals, classified as noise, stopping...." << std::endl;
        m_IsInWindow = true;
        return false;
       }
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: No. of Half points: " << m_halfPtsIndx.size() << " and is signal: " << m_IsSignal << std::endl;
    m_StrIndx = StartIndx(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Start index: " << m_StrIndx << std::endl;
    m_EndIndx = EndIndx(m_voltage, false, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: End index : " << m_EndIndx << std::endl;
    m_IsInWindow = InWindow(m_voltage);
    if (!m_IsInWindow)
       {
        if (m_WvBase->WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " WARNING: Signal ouside of DAQ window, stopping...." << std::endl;
        return false;
       }
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Pulse is within window: " << m_IsInWindow << std::endl;
    std::pair <unsigned int, unsigned int> noisepts = FindNoisePoints(m_voltage);
    if ((int)(noisepts.second - noisepts.first) < 4)
       {    
        if (m_WvBase->WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " WARNING: Too few noise points determined, stopping...." << std::endl;
        return false;
       }
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Using range for noise from " << noisepts.first << " to " << noisepts.second << std::endl;
    auto start_time = std::chrono::high_resolution_clock::now();
    std::pair <double, double> baseline (-1., -1.);
    std::pair <double, double> noise_rms (-1., -1.);
    if (m_WvBase->WfmBase::GetFitMethode() == "rootInt") m_NoiseFtQl = m_WvBase->WfmBase::IterativeFit(m_voltage, baseline, noise_rms, m_noiseFit, m_fitchi2, 
                                                                                                       "GaussInt", noisepts, false, m_FitComp, m_FitCompEvnt);
    else m_NoiseFtQl = m_WvBase->WfmBase::IterativeFit(m_voltage, baseline, noise_rms, m_noiseFit, m_fitchi2,
                                                       "Gauss" ,noisepts, false, m_FitComp, m_FitCompEvnt);
    auto end_time = std::chrono::high_resolution_clock::now();
    auto elapsed_time12 = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
    std::cout << "Fitting duration: " << elapsed_time12 << std::endl;
    if (m_fitchi2 != -1) 
       {
    ...
    if (m_NoiseFtQl == 0)
       {
        ...
        m_noiseErr = noise_rms.second;
          ...
          m_pedestal = m_WvBase->WfmBase::Mean(m_voltage, noisepts.first, noisepts.second);
          m_noise = m_WvBase->WfmBase::Stdev(m_voltage, noisepts.first, noisepts.second);
          if (m_WvBase->WfmBase::GetVerbosity() >= 1) std::cout << __FUNCTION__ << " WARNING: Could not calculate GAUSSIAN noise, reventing to mean and stdv...." << std::endl;
         }
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Noise: " << m_noise << " +/- " << m_noiseErr << " and pedestal: " 
                                                           << m_pedestal << " +/- " << m_pedestalErr << std::endl;
    m_VoltStr = VoltSatur(m_voltage, false);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Saturation test: " << m_VoltStr << std::endl;
    ...
    if (m_pol == pos && !m_VoltStr) m_voltageAdj = PulseAdj(m_voltage, m_pedestal, 1);
            ...
            {
             if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Undetermined polarity for adjustment, stopping...." << std::endl;
            ...
            {
             if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Saturated signal, stopping...." << std::endl;
             m_minTime = TimeMin(m_voltage, m_SnRate, false, false);
             return false;
            }
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Pulse has been adjusted " << m_voltageAdj.size() << std::endl;



//    Region of Interest : Identifying the region of interest, in this case, the rising edge of your signal, is a good practice when you know that the data points of interest are concentrated in that area.This helps reduce the computation time and minimizes the impact of noise in other parts of the signal.
//    Data Selection : Carefully select the portion of the signal that you will use for smoothing and interpolation.Ensure that the selected portion includes the rising edge and extends slightly before and after it to capture the necessary context for accurate smoothing and interpolation.
//    Filter Parameters : Choose appropriate parameters for your Savitzky - Golay smoothing filter, such as the window size and polynomial order, based on the characteristics of the rising edge and the noise level in that region.Smaller window sizes may be suitable for capturing rapid changes in the rising edge.
//    Perform Cubic Spline Interpolation : Interpolate the data points of interest using your cubic spline interpolation method.This will give you the interpolated values.
//    Extrapolation : Keep in mind that when you interpolate points within the selected region, the natural cubic spline will naturally extend beyond the boundaries of that region.This extrapolation can provide a smooth continuation of the curve.
//    Perform Linear Interpolation : Interpolate the same data points using linear interpolation.Linear interpolation is a simpler method where you interpolate between two adjacent data points using a straight line.
//    Calculate the Difference : Compute the absolute difference between the values obtained from cubic spline interpolation and linear interpolation at each point of interest.
//    Set a Threshold : Define a threshold value that determines the acceptable level of difference between the two interpolation methods.This threshold depends on your specific application and the level of precision required.
//    Validation Decision : Keep the spline interpolated value or rever to the linearly interpolated value
    // weight = exp(-(distance ^ 2 / (2 * sigma ^ 2)));


    m_RiseTime = RiseTimeLinear(&m_voltageAdj, m_SnRate);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Rise time calculation " << m_RiseTime << std::endl;
    m_CFDTime = CFDTimeLinear(&m_voltageAdj, m_time, m_fraction);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: CFD time calculation " << m_CFDTime << std::endl;
    m_vAdjCFD = PulseTimeVoltAdj(m_time, &m_voltageAdj, m_fraction);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: TimeWalk corrected pulse: " << m_vAdjCFD.size() << std::endl;
    m_dVdTMax = dVdTMaxLinear(&m_voltageAdj, m_SnRate);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: dV/dT max linear: " << m_dVdTMax << std::endl;
    m_dVdTCFD = dVdTCFDLinear(&m_voltageAdj, m_SnRate, m_fraction);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: dV/dT CFD linear: " << m_dVdTCFD << std::endl;
    m_TriggTime = FirstTimeForVoltage(&m_voltageAdj, m_time, m_trigg);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Trigger time: " << m_TriggTime << std::endl;
    m_CFDToT = CFDToTLinear(&m_voltageAdj, m_time, m_fraction);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: CFD ToT: " << m_CFDToT << std::endl;
    m_TriggToT = TriggToTLinear(&m_voltageAdj, m_time, m_trigg);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Triger ToT: " << m_TriggToT << std::endl;
    if (m_WvBase->WfmBase::GetDoFFT()) 
       {
        m_signalFFT = WfmBase::FFT(&m_voltageAdj, m_SnRate, m_StrIndx, m_EndIndx);
        if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Signal FFT: " << m_signalFFT << std::endl;
        m_noiseFFT = WfmBase::FFT(&m_voltageAdj, m_SnRate, noisepts.first, noisepts.second);
        if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Noise FFT: " << m_noiseFFT << std::endl;
        if (m_signalFFT != -99 && m_WvBase->WfmBase::GetDoTrnsCorr()) m_charge = CollectedCharge(&m_voltageAdj, m_SnRate, GetTrsFromHisto(m_signalFFT, m_trnsHist), m_ampgain, m_StrIndx, m_EndIndx);
        else m_charge = CollectedCharge(&m_voltageAdj, m_SnRate, m_transimp, m_ampgain, m_StrIndx, m_EndIndx);
       }
    else m_charge = CollectedCharge(&m_voltageAdj, m_SnRate, m_transimp, m_ampgain, m_StrIndx, m_EndIndx);
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Collected Charge: " << m_charge << std::endl;
    m_jitter1 = m_noise / m_dVdTCFD;
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Jitter from Noise/dVdT: " << m_charge << std::endl;
    // this needs severe recooding to aocunt for the 10-90% or 20-80% implementation under the assumption of a linear rising edge with constnant slope
    // assuming starting time at 0 = pedestal and at 100% max volt, one should be able to deduct the voltage at 10%-90% or 20%-80%.
    m_jitter2 = m_RiseTime*m_noise / m_maxVolt;
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: Jitter from RiseTime/pMax: " << m_charge << std::endl;

    if (m_WvBase->WfmBase::GetVerbosity() >= 3) WaveForm::dump();
    ...
    if (voltage != NULL)
      { 
       ...
       time = &m_intime;
         ...
         if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " WARNING: Rate set but no voltage vector found. Cannot establish time vector, remember to do it before calculations!" 
                                                               << std::endl;
        }
   return time;
}
...
Long64_t WaveForm::GetSnRate()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Rate not set!!" << std::endl;
    return -1;
}
// --------------------------------------------------------------------------------------------------------------
...
{
    if (m_basepos == undet && m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Base Position not established!!" << std::endl;
    return m_basepos;
}
// --------------------------------------------------------------------------------------------------------------
...
TH2D* WaveForm::GetTrnsHist()
{
    if (m_trnsHist) return m_trnsHist;
          ...
          m_WvBase->WfmBase::SetDoTrnsCorr(false);
          return nullptr;
         }
}
// --------------------------------------------------------------------------------------------------------------
...
std::vector<double> WaveForm::GetAdjVoltage()
{
    if (m_voltageAdj.size() == 0)
       {
        if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Voltage adjustment for pedestal not yet performed!" << std::endl;
...
std::vector<double> WaveForm::GetTimeAdjVolt(float fraction)
{
    if (m_vAdjCFD.size() == 0)
       { 
        if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Time adjustment for CFD not yet performed!" << std::endl;
        return std::vector<double>();
       }
    else if (fraction == -99) return m_vAdjCFD;
    else if (fraction > 0 && fraction < 1) return PulseTimeVoltAdj(m_time, &m_voltageAdj, fraction);
    else {
          if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Ubacceptable CFD Value, using " << GetCFDfraction() << " ..." << std::endl;
          return m_vAdjCFD;
         }
}
// --------------------------------------------------------------------------------------------------------------
int WaveForm::GetMaxIndx()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << "WARNING: Maximum not defined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
...
double WaveForm::GetMaxErr()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Maximum time not defined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
std::vector<int> WaveForm::GetHalfPoints()
{
    if (m_halfPtsIndx.size() > 0) return m_halfPtsIndx;
          ...
          if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << "WARNING: Half Points not calculated!!" << std::endl;
...
int WaveForm::GetMinIndx()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Minimum not defined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
...
int WaveForm::GetStrIndx()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Signal start not defined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
...
double WaveForm::GetNoise()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Noise not calculated!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetNoiseErr()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Noise uncertenty not calculated!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetPedestal()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pedestal not determined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetPedestalErr()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pedestal uncertenty not determined!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetCharge()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Charge has not been calculated!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetRiseTime(float bottom, float top)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Rise time has not been estimated!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetCFDTime(float fraction)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: CFD time has not been estimated!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetTriggTime(float trigg)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Trigger time has not been estimated!!" << std::endl;
    return -1;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetdVdTMax()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Maximum dV/dT has not been computed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetdVdTCFD(float fraction, int ndif)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: dV/dT at CFD value has not been computed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetCFDToT(float fraction)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: ToT for CFD value has not been computed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetTriggToT(float trigg)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: ToT for trigger value has not been computed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetSignalFFT()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Fast Fourier Transform for signal not performed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetNoiseFFT()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Fast Fourier Transform for noise not performed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetJitterNdVdT(float fraction, int ndif)
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Jitter [noise/dVdt] calculation not performed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetJitterRiseSNR()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Jitter [Rise Time/SNR] calculation not performed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetFrequency(int start, int stop)
{
    if (m_signalFFT != -99)
       {
        if (start > 0 && stop > 0 && start < stop) return m_WvBase->WfmBase::FFT(&m_voltageAdj, m_SnRate, start, stop);
        else {
              std::cout << __FUNCTION__ << " WARNING: Unacceptable values for start and stop, abording..." << std::endl;
              return -1.;
            }
       }
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: FFT not yet performed!!" << std::endl;
    return -1.;
}
// --------------------------------------------------------------------------------------------------------------
bool WaveForm::GetSaturation()
{
    ...
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Saturation test not performed!!" << std::endl;
    return false;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::GetFitComp(unsigned int indx1, bool compevnt, unsigned int indx2, unsigned int indx3)
{
    if (!compevnt && indx1 < 6) return m_FitComp[indx1];
    else if (compevnt && indx1 < 5)
            {
             if (indx2 < 6 && indx3 < 7) return m_FitCompEvnt[indx1][indx2][indx3];
             else {
                   std::cout << __FUNCTION__ << " WARNING: Incorect array index for fit comparison 2D-space bin, indx1 < 6 and indx2 < 7!" <<std::endl;
                   return -99.;
                  }
            }
    else {
          if (compevnt) std::cout << __FUNCTION__ << " WARNING: Incorect array index for fit comparison 2D histogram, indx < 5!" << std::endl;
          else std::cout << __FUNCTION__ << " WARNING: Incorect array index for fit comparison quantities, indx < 6!" << std::endl;
          return -99.;
         }
}
// --------------------------------------------------------------------------------------------------------------
WaveForm::basepos WaveForm::FindPosition(const std::vector<double> &w)
{
    basepos position;
    bool zerocr = false;
...
// --------------------------------------------------------------------------------------------------------------
WaveForm::polarity WaveForm::FindPolarity(const std::vector<double> &w)
{
    polarity pol = undef;
    std::vector<double> firstder = m_WvBase->WfmBase::Derivate(w, 3);
    // 0-> Positive
    // 1-> Negative
    // 2-> Undefined
    if (firstder.size() != 0)
       { 
        std::vector<double> secordr = m_WvBase->WfmBase::Derivate(&firstder, 1);
        if (secordr.size() != 0)
           { 
            ...
            if (maxvolt.size() != 0 && minvolt.size() != 0)
               {
                ...
                m_maxavg = m_WvBase->WfmBase::Mean(maxvolt);
                m_minavg = m_WvBase->WfmBase::Mean(minvolt);
                double maxstdev = m_WvBase->WfmBase::Stdev(&maxvolt, -1, -1, m_maxavg);
                double minstdev = m_WvBase->WfmBase::Stdev(&minvolt, -1, -1, m_minavg);
                ...
                double wfmavg =  m_WvBase->WfmBase::Mean(w);
                ...
                for (unsigned int i = 0; i < 4; i++)
                    {
                     ...
                     if (m_WvBase->WfmBase::GetVerbosity() > 2) std::cout << __FUNCTION__ << " INFO:" << i << " : " << hpl2 << "\t" 
                ...
                if (posweight > negweight) pol = pos;
                      ...
                      if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Cannot determine polarity with available info!!" << std::endl; 
                      pol = undef;
                     }
                if (m_WvBase->WfmBase::GetVerbosity() > 1) std::cout << __FUNCTION__ << " INFO: Polarity is : " << pol << ", Positive weight: " 
                                                                      << posweight << ", Negative Weight: " << negweight << std::endl;
               }
            else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Cannot determine polarity, no maxima or minima found!" << std::endl;
           }
        else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Cannot determine polarity, second derivative size too small!" << std::endl;
       }
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Cannot determine polarity, first derivative size too small!" << std::endl;
    return pol;
}
// --------------------------------------------------------------------------------------------------------------
// Function to find the RELATIVE to polarity maxima of the pulse and return the inddex of that value
int WaveForm::VoltMaxIndx(const std::vector<double> &w, bool adj, bool poldef)
{
    int ind_max = -1;
    int npoints = w->size();
    double pmax;
...
double WaveForm::VoltMax(const std::vector<double> &w, bool adj, bool poldef)
{
...
// --------------------------------------------------------------------------------------------------------------
// Return uncertenty of the maximum voltage
double WaveForm::VoltMaxErr(const std::vector<double> &w, bool adj)
{
...
double WaveForm::TimeMax(const std::vector<double> &w, Long64_t snrate, bool adj, bool poldef)
{
    if (m_maxIndx == -99) m_maxIndx = VoltMaxIndx(w, adj, false);
...
int WaveForm::VoltMinIndx(const std::vector<double> &w, bool adj, bool poldef)
{
    int ind_min = -1;
    int npoints = w->size();
...
double WaveForm::VoltMin(const std::vector<double> &w, bool adj, bool poldef)
{
...
double WaveForm::TimeMin(const std::vector<double> &w, Long64_t snrate, bool adj, bool poldef)
{
    if (m_minIndx == -99) m_minIndx = VoltMinIndx(w, adj, poldef);
...
std::vector<int> WaveForm::FindHalfPoints(const std::vector<double> &w, bool adj)
{
    if (m_maxVolt == -99) m_maxVolt = VoltMax(w, adj);
    if (m_minVolt == -99) m_minVolt = VoltMin(w, adj);
    double halfvalue = 0.0;
    std::vector<int> halfPtsIndx;
    if (m_WvBase->WfmBase::GetVerbosity() > 1) std::cout << __FUNCTION__ << "======================================================" << std::endl;
    ...
    // -20 is noise, -10 is signal, 1 is undetermined
    ...
    if (m_WvBase->WfmBase::GetVerbosity() > 1) std::cout << __FUNCTION__ << " HalfPoints Size: " << halfPtsIndx.size() << " IsSignal criteria: " << halfPtsIndx.at(halfPtsIndx.size()-1) << std::endl;
    return halfPtsIndx;
}
// --------------------------------------------------------------------------------------------------------------
...
bool WaveForm::VoltSatur(const std::vector<double> &w, bool poldef)
{
    bool satur = false;
    ...
    if (m_halfPtsIndx.size() >= 2)
       {
        if (endindx == -1 && (m_StrIndx > (int)w->size()/2))
           {
            if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pulse within window but located at the higher extremity!" << std::endl;
            window = true;
           }
        if (m_StrIndx == -1 && (endindx > (int)w->size()/2))
           {
            if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pulse outside window, located at the lower extremity, rise time calculation will fail!" << std::endl;
            window = false;
           }
        else window = true;
       }
             ...
             if (endindx == -1 && (m_StrIndx > (int)w->size() / 2))
                {
                 if (m_maxIndx == (int)w->size()) 
                    { 
                      window = false;
                      if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pulse rising edge ouside window at high side!" << std::endl;
                    }
                 else {
                       window = true;
                       if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Pulse rising edge within window but located at the higher extremity!" << std::endl;
                      }
                }
...
int WaveForm::StartIndx(const std::vector<double> &w, bool adj, bool poldef)
{
    int istart = -1;
    double basevolt = 0.0;
    if (poldef && m_pol == undef) m_pol = FindPolarity(w);
    ...
    if (istart == -1 && m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Signal start index not found!! " << std::endl;
    return istart;
}
// --------------------------------------------------------------------------------------------------------------
// Without adjustment, find the point that is 20% higher than the lowest detectable voltage of the pulse
int WaveForm::EndIndx(const std::vector<double> &w, bool adj, bool poldef)
{
    int iend = -1;
    double basevolt = 0.0;
    if (poldef && m_pol == undef) m_pol = FindPolarity(w);
    ...
    if (iend == -1 && m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Signal end index not found!! " << std::endl;
    return iend;
}
// --------------------------------------------------------------------------------------------------------------
...
std::pair <unsigned int, unsigned int> WaveForm::FindNoisePoints(const std::vector<double> &w)
{
    int npoints = w->size();
    if (m_StrIndx == -99) m_StrIndx = StartIndx(w, false, false);
    ...
    if (ceil(0.8*m_StrIndx) > ceil(0.2*m_StrIndx) && ceil(0.8*m_StrIndx) >= 10) noisepoints.second = ceil(0.8*m_StrIndx);
          ...
          else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " WARNING: Noise Determination Failed, starting point"
                                                                     << noisepoints.first << ", ending point: " << ceil(0.9*m_StrIndx) << "!!!!" << std::endl;
         }
    return noisepoints;
}
// --------------------------------------------------------------------------------------------------------------
std::vector<double> WaveForm::PulseAdj(const std::vector<double> &w, double baseline, int factor)
{
    int npoints = w->size();
    ...
    if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: After pedestal subtraction and inversion : baseline " << baseline 
                                                           << ", corrected vector size: " << wadj .size() << ", factor " << factor << std::endl;

    return wadj;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::CollectedCharge(const std::vector<double> &w, Long64_t snrate, float transimp, float ampgain, int start, int stop)
{
    ...
    if (m_WvBase->WfmBase::GetVerbosity() >= 2)  std::cout << __FUNCTION__ << " INFO: Charge Calculation-> charge " << charge << ", gain: " << ampgain 
                                                            << ", transimpedence: "<< transimp  << ", timebin size: " << time_difference << std::endl;
    return charge; 
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::FirstTimeForVoltage(const std::vector<double> &w, std::vector<double> *t, double volt)
{
    double tar = -1.;
    int ind_tar = -1;
    ...
    if (ind_tar != -1 && ind_tar != m_maxIndx)
       {
        ...
        if (tar < t->back() && tar > t->front()) return tar;
              ...
              if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed locating time for voltage at " << volt << " trigger!" << std::endl;
              return -1.;
             }
       }
    else if (ind_tar == m_maxIndx)
            {
             if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Voltage value " << volt << " is higher than pulse maximum!" << std::endl;
             return -1.;
            }
    else {
          if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to find position where voltage " << volt << " first occurrs!" << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::CFDTimeLinear(const std::vector<double> &w, std::vector<double> *t, float fraction)
{
...
// --------------------------------------------------------------------------------------------------------------
std::vector<double> WaveForm::PulseTimeVoltAdj(std::vector<double> *t, const std::vector<double> &w, float fraction)
{
    ...
    if (indxCFD != -99)
       {
        ...
        if (m_WvBase->WfmBase::GetVerbosity() >= 2) std::cout << __FUNCTION__ << " INFO: After timewalk correction : CFD fraction " 
                                                               << fraction << ", corrected vector size: " << vadjCFD.size() << std::endl;
       }
    else if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Failed to find time index of CFD time: " << time << std::endl;

    return vadjCFD;
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::dVdTMaxLinear(const std::vector<double> &w, Long64_t snrate)
{
    ...
    if (dvdt != 0) return dvdt*snrate;
          ...
          if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to calculate maximum dV/dt value!" << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::dVdTCFDLinear(const std::vector<double> &w, Long64_t snrate, float fraction, int ndif)
{
    ...
    if (ifraction != -99)
       {
        ...
        if (dvdt != 0) return dvdt;
              ...
              if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Failed to calculate dV/dT at a CFD of " << fraction << std::endl;
              return -1.;
             }
       }
    else {
          if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Failed to determine first point at a CFD of " << fraction << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::RiseTimeLinear(const std::vector<double> &w, Long64_t snrate, float top, float bottom)
{
    ...
    if (bottom_indx != -1 && top_indx != -1 && top_indx > bottom_indx)
       { 
        ...
        if (fabs(ttop - tbottom) > 0) return fabs(ttop - tbottom); // rise time
              ...
              if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Linear interpolation failed!" << std::endl;
              return -1.;
             }
       }
    else {
          if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to determine upper and lower values" << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::SecondTimeForVoltage(const std::vector<double> &w, std::vector<double> *t, double volt)
{
    ...
    if (ind_t != -1 && ind_t != m_maxIndx)
       { 
        ...
        if (tar != -1 && tar < t->back() && tar > t->front()) return tar; // Time over threshold
              ...
              if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to calculate second time for voltage: " << volt << std::endl;
              return -1.;
             }
       }
    else if (ind_t == m_maxIndx)
            {
             if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Voltage value " << volt << " is larger than pulse maximum!" << std::endl;
             return -1.;
            }
    else {
          if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to determine second index for voltage " << volt << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::CFDToTLinear(const std::vector<double> &w, std::vector<double> *t, float fraction)
{
    if (m_EndIndx == -99) m_EndIndx = EndIndx(w, true);
    if (m_maxIndx == -99) m_maxIndx = VoltMaxIndx(w, true);
    if (m_maxVolt == -99) m_maxVolt = VoltMax(w, true);
    ...
    if (nd_time != -1 && st_time != -1 && nd_time != st_time) return abs(nd_time - st_time);
          ...
          if (m_WvBase->WfmBase::GetVerbosity() > 0 ) std::cout << __FUNCTION__ << " ERROR: Failed to calculate CFD ToT!" << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
double WaveForm::TriggToTLinear(const std::vector<double> &w, std::vector<double> *t, double trig)
{
    if (m_EndIndx == -99) m_EndIndx = EndIndx(w, true);
    if (m_maxIndx == -99) m_maxIndx = VoltMaxIndx(w, true);
    double t1 = -99.;
    ...
    if (t1 != -99 && t2 !=-1 && t1 != t2) return fabs(t2 - t1); // Time over threshold
          ...
          if (m_WvBase->WfmBase::GetVerbosity() > 0) std::cout << __FUNCTION__ << " ERROR: Failed to calculate Trigger ToT!" << std::endl;
          return -1.;
         }
}
// --------------------------------------------------------------------------------------------------------------
