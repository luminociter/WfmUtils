...
bool LGADBase::ConvertData()
{
    ...
    m_channels.clear();
    m_convert = false;
    if (m_instrument == LeCroyWRBin || m_instrument == Sampic || m_instrument == TestBeamBin || m_instrument == TestBeamBin1 || m_instrument == TestBeamBin2)
       {
        if (m_ext.IsNull()) 
           {
            ...
            else if (m_instrument == TestBeamBin1 || m_instrument == TestBeamBin2 || m_instrument == TestBeamBin) m_convert = WriteTestBeamBinary(m_datadir.Data(), m_dataname.Data(), "dat", m_evnt1, m_evnt2);
           }
        else {
              if (m_instrument == LeCroyWRBin) m_convert = WriteLecroyBinary(m_datadir.Data(), m_dataname.Data(), m_ext.Data(), m_evnt1, m_evnt2);
              else if (m_instrument == TestBeamBin1 || m_instrument == TestBeamBin2 || m_instrument == TestBeamBin) m_convert = WriteTestBeamBinary(m_datadir.Data(), m_dataname.Data(), m_ext.Data(), m_evnt1, m_evnt2);
...
// A seperate txt file is created per channel and per event.
...
// --------------------------------------------------------------------------------------------------------------
// Probably depricated but still coreresponds to some old BNL data types
             ...
             {        
              ...
              for (unsigned int j = 0; j < m_nchan; j++)
                  ...
                  for (unsigned int w = 0; w < (l_npoints[m_nchan*d+j]); w++)
                      {
                       if ((w > (l_npoints[m_nchan*d + j]-4)) && (((fileBuf[w+offset] * yscale[m_nchan*d+j]) - yoffset[m_nchan*d+j]) > 0.04)) break;
                       else if ((w < 10) && ((((fileBuf[w+offset] * yscale[m_nchan*d+j]) - yoffset[m_nchan*d+j]) > 0.04) || 
                                (((fileBuf[w+offset] * yscale[m_nchan*d+j]) - yoffset[m_nchan*d+j]) < -0.04))) continue;
                       else {
...
// 31 dasy chaned scopes with common clock and syncronistion. Works with Agilent and Tektronix scope with automatic selection
bool LGADBase::WriteTestBeamBinary(const char* dir, const char* name, const char* ext, unsigned int evt1, unsigned int evt2)
{
    std::vector<unsigned int> nevt_seq;  // internal, number of events per sequence, identical for all channels
    static FILE* fd_file = NULL;  // internal
    char* fileBuf;  // internal
    ...
    std::vector<unsigned int> dattype;
    std::vector<unsigned int> l_npoints;
    std::vector<double> l_triggtimeVec;
    std::vector<Long64_t> l_srate;
    char ifname[2048];
    ...
    if (fd_file == NULL)
       {
        ...
        fd_file = fopen(ifname, "r");
        if (fd_file == NULL) { std::cout << __FUNCTION__ << " ERROR: Failed to open txt file: " << ifname << std::endl; return false; }
        else m_instrument = TestBeamBin2;
       }
    else m_instrument = TestBeamBin1;
...
#endif
    if (size <= 0) { std::cout << __FUNCTION__ << " ERROR: txt file is zero size, skipping..." << std::endl; return false; }
    while (!readhead && fgets(ifname, sizeof(ifname), fd_file))
          {
           unsigned int nel, npts, nev, xorg;
           float yinc, yorg;
           double sr;
                    ...
                    if (!(nel == 9 && npts > 0 && n_bytes > 0))
                       {
                        if (m_nchan == 0) // failed right from the start, no channels were read
                           {
                            std::cout << __FUNCTION__ << " ERROR: Invalid parameters from header file!" << std::endl; 
                            return false; 
                           }
                        else {
                              readhead = true;
...
#ifdef _WIN32
                              if (nrec+1 == sizeof(ifname)) _fseeki64(fd_file, _ftelli64(fd_file)-nrec-4, 0);
                              else if (nrec + 1 < sizeof(ifname)) _fseeki64(fd_file, _ftelli64(fd_file)-nrec-5, 0);
#else
                              if (nrec + 1 == sizeof(ifname)) fseek(fd_file, ftell(fd_file) - nrec - 4, 0);
                              else if (nrec + 1 < sizeof(ifname)) fseek(fd_file, ftell(fd_file) - nrec - 5, 0);
#endif
                              nrec = 0;
                             }
                       } 
           ...
           if ((m_instrument == TestBeamBin1 && m_nchan == 0) || (m_instrument == TestBeamBin2 && !readhead))
              {
               yscale.push_back(yinc);
               yoffset.push_back(yorg);
               l_npoints.push_back(npts);
               l_srate.push_back((double)1/sr);
               if (m_instrument == TestBeamBin1)
                  {
    ...
    // Reading out the events and dates/times from header file
    if (readhead)
       { 
        double trigtime_first = 0.0; // time of the first event
        m_trigtime = 0.0;
        unsigned int nel, year, month;
        unsigned int fact = 0;
        struct tm timeinfo;
        long double milisec, milisec1;
#ifdef _WIN32
        lCurPos = _ftelli64(fd_file);
        while (_ftelli64(fd_file) < size)
#else
        lCurPos = ftell(fd_file);
        while (ftell(fd_file) < size)
#endif
              {
...
#ifdef _WIN32
                    else if (_ftelli64(fd_file) == size) break; // we are at the end of the file
#else
                    else if (ftell(fd_file) == size || ifname[i] == '\n') break; // we are at the end of the file or line
#endif
                   }
               nel = sscanf(ifname, "\"%i: %2u.%2u.%4u.%2u:%2u:%2u.%Lf\"", &nrec, &timeinfo.tm_mday, &month, &year, &timeinfo.tm_hour, &timeinfo.tm_min, &timeinfo.tm_sec, &milisec);
               if (!(nel == 8 && nrec > 0 && milisec > 0 && year > 0))
                  { 
                   std::cout << __FUNCTION__ << " ERROR: Invalid parameters from header file for event date/time!" << std::endl; 
                   return false; 
                  }               
               timeinfo.tm_year = year - 1900;
               ...
               if (nrec == 1) // trigger time from first event in the sequence
                  {
                   fact = 0;
                   trigtime_first = m_trigtime;
                   while (milisec > 1) { milisec = milisec / ((double)10); fact++; }
                   milisec1 = milisec;
                  }
               m_trigtime = (long long int)m_trigtime - (long long int)trigtime_first;
               if (m_trigtime < 0) { while (m_trigtime < 0) m_trigtime = m_trigtime + 3600; }
               ...
               else m_trigtime = m_trigtime + ((long double)(milisec / pow(10, fact)) - (long double)milisec1);
               l_triggtimeVec.push_back(m_trigtime);
               if (nrec != 1) nevt_seq.at(nevt_seq.size() - 1) = nrec; // update the number of events for the sequence
               else nevt_seq.push_back(nrec); // add an entry to the vector, start a new sequence
               memset(&ifname[0], 0, sizeof(ifname)); // clear the char buffer before reusing it
              }
       }
    if (m_instrument == TestBeamBin1)
       {
        if (m_nchan == 0) m_nchan = nrec;
        if (nevt_seq.size() != (nrec / m_nchan)) 
           {
            std::cout << __FUNCTION__ << " ERROR: Unable to correctly determine number of cycles and total number of events" << std::endl; 
            return false; 
           }
       }
    ievent = 0;
    ...
    lCurPos = 0; // reuse the integer

    // Local vectors to store number of bytes and number of points digit
    std::vector<unsigned int> l_bytes;
    std::vector<unsigned int> l_digits;
    l_bytes.reserve(m_nchan);
    l_digits.reserve(m_nchan);
    for (unsigned f = 0; f < m_nchan; f++)
        { 
         if (m_instrument == TestBeamBin1) lCurPos += (l_npoints.at(f) + 1) * 2 * ievent;
         else if (m_instrument == TestBeamBin2)
                 {
                  scope = 0; // reuse the integer
                  nrec = l_npoints.at(f) + xoffset.at(f); // reuse the integer
                  if (dattype.at(f) < 200) l_bytes.push_back(1);
                  else if (dattype.at(f) > 200) l_bytes.push_back(2);
                  else {
                        std::cout << __FUNCTION__ << " ERROR: Incorect data size for channel " << m_channels.at(f) << std::endl;
                        return false;
                       }
    ...
    if (evt1 > 0 && evt1 > ievent)
       {
        if (evt2 > 0 && ((evt2 - evt1) < ievent)) { evt2 = evt2 - evt1; evt1 = 0;}
        else { evt1 = 0; evt2 = ievent; }
        std::cout << __FUNCTION__ << "          Adjusting start / stop events to available data: " << evt1 << " - " << evt2 << std::endl;
       }
    else if (evt1 > 0 && evt2 > 0 && ((evt2-evt1)>ievent))
       {   
        evt2 = ievent - evt1;
        std::cout << __FUNCTION__ << "          Adjusting stop event to available events " << evt2 << std::endl;
       }
    else if (evt1 == 0 && evt2 > 0 && (evt2 > ievent))
            { 
             evt2 = ievent;
             std::cout << __FUNCTION__ << "          Adjusting stop event to available events " << evt2 << std::endl;
            }
    ...
    for (unsigned int d = 0; d < nevt_seq.size(); d++)
        {
         ...
         for (unsigned f = 0; f < m_nchan; f++)
             {
              if (m_instrument == TestBeamBin1) lCurPos += (l_npoints.at(f) + 1) * 2 * nevt_seq.at(d);
              else if (m_instrument == TestBeamBin2) lCurPos += ((l_npoints.at(f) * l_bytes.at(f)) + 3 + l_digits.at(f)) * nevt_seq.at(d) - 1;
             }
         fileBuf = (char*)malloc(lCurPos);
         size_t result = fread(fileBuf, lCurPos, 1, fd_file);
         if (fileBuf == NULL || result != 1) { std::cout << __FUNCTION__ << " ERROR: Buffer is empty, check data file!!!" << std::endl; return false; }
         // Start Converting events in buffer
         for (unsigned int i = 0; i < nevt_seq.at(d); i++)
             { 
              if ((int)m_event < evt1) continue;
              if (evt2 != 0 && (int)m_event >= evt2) break;
              ...
              for (unsigned int j = 0; j < m_nchan; j++)
                  {  
                   ...
                   if ((int)m_event == evt1)
                      { 
                       if (m_instrument == TestBeamBin1) m_scale[j] = yscale[j] / 2.64674e-4;
                       else { 
                             if (dattype.at(j) < 200) m_scale[j] = yscale[j] / 1.5625e-4; // better ADC than Agilent
                             else if (dattype.at(j) > 200) m_scale[j] = yscale[j] / 4e-2;
                            }
                   ...
                   for (unsigned int h = 0; h < j; h++) 
                       {
                        if (m_instrument == TestBeamBin1) lCurPos += (l_npoints.at(h)+1)*2*nevt_seq.at(d); 
                        else if (m_instrument == TestBeamBin2) lCurPos += ((l_npoints.at(h)* l_bytes.at(h))+3+ l_digits.at(h))*nevt_seq.at(d)-1;
                       }
                   ...
                   if (m_instrument == TestBeamBin1) lCurPos += i*2*(l_npoints.at(j) + 1);
                   if (m_instrument == TestBeamBin2) lCurPos += i *((l_npoints.at(j) * l_bytes.at(j)) + 3 + l_digits.at(j));
