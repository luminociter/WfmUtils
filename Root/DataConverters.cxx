...
bool LGADBase::WriteTestBeamBinary(const char* dir, const char* name, const char* ext, unsigned int evt1, unsigned int evt2)
{
...
#ifdef _WIN32
                              if ((nrec+1) == (int)(sizeof(ifname))) _fseeki64(fd_file, _ftelli64(fd_file)-nrec-4, 0);
                              else if ((nrec+1) < (int)(sizeof(ifname))) _fseeki64(fd_file, _ftelli64(fd_file)-nrec-5, 0);
#else
                              if ((nrec+1) == (int)(sizeof(ifname))) fseek(fd_file, ftell(fd_file)-nrec, 0);
                              else if ((nrec+1) < (int)(sizeof(ifname))) fseek(fd_file, ftell(fd_file)-nrec, 0);
#endif
                              nrec = 0;
                             }
                       } 
       ...
       { 
        ...
        while (ftell(fd_file) < (int)size)
#endif
              {
               ...
               strcpy(ifname, (LGADBase::trim(std::string(ifname), "\"")).c_str());
    ...
    if (size != lCurPos)
       { 
        std::cout << __FUNCTION__ << " ERROR: Data file size not corresponding to expected value!" << std::endl; 
        if (size > lCurPos) std::cout << "                                     Size bigger than estimated, will try to recover expected events from header but CHECK DATA!!!!" << std::endl;
        else return false; 
       }
