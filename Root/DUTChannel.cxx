...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(WfmBase *tBase)
{
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(int channelID, std::string name, WfmBase *tBase)
{
    m_ChWaveform = new WaveForm(tBase);
    m_ChBase = tBase;
    ChannelInit(m_ChBase);
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(int channelID, WfmBase *tBase)
{
    m_ChBase = tBase;
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(std::string name, WfmBase *tBase)
{
    m_ChWaveform = new WaveForm(tBase);
    m_ChBase = tBase;
    ChannelInit(m_ChBase);
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(int channelID, std::string name, AqBoard board, SecStage ampl, float fraction, float trigger, WfmBase *tBase)
{
    m_ChWaveform = new WaveForm(tBase);
    m_ChBase = tBase;
    ChannelInit(m_ChBase);
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(int channelID, std::string name, AqBoard board, SecStage ampl, double cap, float fraction, float trigger, WfmBase *tBase)
{
    m_ChWaveform = new WaveForm(tBase);
    m_ChBase = tBase;
    ChannelInit(m_ChBase);
...
// --------------------------------------------------------------------------------------------------------------
DUTChannel::DUTChannel(DUTChannel* tChannel)
{
    m_ChBase = tChannel->GetBase();
    m_board = tChannel->GetBoard();
    m_amp = tChannel->GetAmp();
    m_channelName = tChannel->GetChName();
    m_ChCap = tChannel->GetChCap();
    m_channelID = tChannel->GetChID();
    m_ChRate = tChannel->GetChRate();
    m_ChTransimp = tChannel->GetChTransImp() ;
    m_ChAmpgain = tChannel->GetChAmpGain();
    m_ChBinSz = tChannel->GetChBinSz();
    m_ChLowLim = tChannel->GetChLowLim();
    m_ChUpLim = tChannel->GetChUpLim();
    m_ChFraction = tChannel->GetChFract();
    m_ChTrigg = tChannel->GetChFTrigg();
    m_ChVoltCut = tChannel->GetVoltCut();
    m_ChChargeCut = tChannel->GetChargetCut();
    m_ChNoiseCut = tChannel->GetNoiseCut();
    m_ChJitterCut = tChannel->GetJitterCut();
}
// -----------------------------------------------------------------------------------------------------------
bool DUTChannel::SetChName(std::string name)
{
    if (name.length() != 0 && !(WfmBase::trim(name, " ")).empty()) { m_channelName = name; return true; }
    else {
...
{   
    ...
    if (m_ChBase->WfmBase::GetDoTrnsCorr() && (m_ChBase->WfmBase::GetTransHistos()).size() != 0) SetTrnsHist(m_board, m_ChCap, m_ChBase->WfmBase::GetTransHistos());
}
// --------------------------------------------------------------------------------------------------------------
bool DUTChannel::SetTrnsHist(AqBoard Brd, double Cap, std::vector<TrCrHist> HistVec)
{
   ...
   if (found) return true;
         ...
         m_ChBase->WfmBase::SetDoTrnsCorr(false);
    ...
    if (ampl == MinCircuits || ampl == Particulars || ampl == CIVIDEC || ampl == none)
       {
        ...
        if (ampl == MinCircuits) SetChAmpGain(10);
          ...
          m_amp = MinCircuits;
          std::cout << __FUNCTION__ << " WARNING: second stagne mplifier " << ampl
                    << " not allowed. Allowed values are MinCircuits, Particulars, CIVIDEC and none. Falling back to "
                    << m_amp << std::endl;
        }
}
// --------------------------------------------------------------------------------------------------------------
void DUTChannel::SetChBinSz(double size)
{
    if (size > 0 && size < 10000) m_ChBinSz = size;
    else {
          std::cout << __FUNCTION__ << " WARNING: Invalid tracking bin size for channel " << m_channelName << ", setting to 10mm..." << std::endl;
          m_ChBinSz = 10000;
         }
}
// --------------------------------------------------------------------------------------------------------------
void DUTChannel::SetChBinShape(std::string shape)
{
    if (shape != "square" || shape != "circle")
       {
        std::cout << __FUNCTION__ << " WARNING: Invalid tracking bin shape for channel " << m_channelName << ", setting to square..." << std::endl;
        m_ChBinShape = "square";
       }
    else m_ChBinShape = shape;
}
// --------------------------------------------------------------------------------------------------------------
void DUTChannel::SetChLowLim(std::pair <double, double> lowlim)
{
    if (lowlim.first >= -10 && lowlim.first <= 10 && lowlim.second >= -10 && lowlim.second <= 10)
       {
        if (m_ChUpLim.first !=-99 && m_ChUpLim.second != -99)
           {
            if (m_ChUpLim.first < lowlim.first && lowlim.second < m_ChUpLim.second) m_ChLowLim = lowlim;
            else {
                  std::cout << __FUNCTION__ << " WARNING: Channel " << m_channelName;
                  if (m_ChUpLim.first >= lowlim.first)
                     {
                      std::cout << " x low limit (" << lowlim.first << ") set lower than high limit (" << m_ChUpLim.first << "), setting to 10!"<< std::endl;
                      m_ChLowLim = std::make_pair(10, lowlim.second);
                     }
                  else {
                        std::cout << " y low limit (" << lowlim.second << ") set higher than high limit (" << m_ChUpLim.second << "), setting to -10!" << std::endl;
                        m_ChLowLim = std::make_pair(lowlim.first, -10);
                       }
                 }
           }
        else m_ChLowLim = lowlim;
       }
    else {
          std::cout << __FUNCTION__ << " WARNING: Channel " << m_channelName;
          if (lowlim.first < -10 || lowlim.first > 10)
             {
              std::cout << " x upper limit out of bounds (-10, 10), ignoring...." << std::endl;
              m_ChLowLim =  std::make_pair(-10, 10);
             }
          else {
...
// --------------------------------------------------------------------------------------------------------------
void DUTChannel::SetChUpLim(std::pair <double, double> uplim)
{
    if (uplim.first >= -10 && uplim.first <= 10 && uplim.second >= -10 && uplim.second <= 10)
       {
        if (m_ChLowLim.first != -99 && m_ChLowLim.second != -99)
           {
            if (m_ChLowLim.first > uplim.first && uplim.second > m_ChLowLim.second) m_ChUpLim = uplim;
            else {
                  std::cout << __FUNCTION__ << " WARNING: Channel " << m_channelName;
                  if (m_ChLowLim.first <= uplim.first)
                     {
                      std::cout << " x high limit (" << uplim.first << ") set higher than low limit (" << m_ChLowLim.first << "), setting to -10!"<< std::endl;
                      m_ChUpLim = std::make_pair(-10, uplim.second);
                     }
                  else {
                        std::cout << " y high limit (" << uplim.second << ") set lower than low limit (" << m_ChLowLim.second << "), setting to 10!" << std::endl;
                        m_ChUpLim = std::make_pair(uplim.first, 10);
                       }
                 }
           }
        else m_ChUpLim = uplim;
       }
    else {
          std::cout << __FUNCTION__ << " WARNING: Channel " << m_channelName;
          if (uplim.first < -10 || uplim.first > 10)
             {
              std::cout << " x upper limit out of bounds (-10, 10), ignoring...." << std::endl;
              m_ChUpLim = std::make_pair(-10, 10);
             }
          else {
                std::cout << " y upper limit out of bounds (-10, 10), ignoring..." << std::endl;   
                m_ChUpLim = std::make_pair(-10, 10);
...
// --------------------------------------------------------------------------------------------------------------
bool DUTChannel::SetChargeCut(double ChargeCut)
{
    if (ChargeCut > 0) { m_ChChargeCut = ChargeCut; return true;}
    else {
          std::cout << __FUNCTION__ << " WARNING: Zero or negative max votlage cut values not allowed, ignoring... ";
          m_ChChargeCut = -99.;
          return false;
...
// --------------------------------------------------------------------------------------------------------------
bool DUTChannel::SetEvntVectQuant(double quant, unsigned int bin, unsigned int mode)
{
    if (bin < 19)
       {
        if (mode == 1) (m_ChCFDTime[bin]).push_back(quant);
        else if (mode == 2) (m_ChDVDTCFD[bin]).push_back(quant);
        else if (mode == 3) (m_ChCFDToT[bin]).push_back(quant);
        else if (mode == 4) (m_ChJitNdVdT[bin]).push_back(quant);
        return true;
       }
    else {
          std::cout << __FUNCTION__ << " ERROR: Inalid CFD time bin, " << bin << ". Accepter values between 0 - 19!" << std::endl;
          return false;
         }
}
// --------------------------------------------------------------------------------------------------------------
...
double DUTChannel::GetFitChi2(std::string fit, int i)
{
    ...
    if (fit == "MaxIndx") return m_ChFitMaxIndxChi2;
    ...
    else if (fit == "SigFFT" && m_ChBase->WfmBase::GetDoFFT()) return m_ChSignalFFTChi2;
    else if (fit == "NoiseFFT" && m_ChBase->WfmBase::GetDoFFT()) return m_ChNoiseFFTChi2;
          ...
          if ((fit == "SigFFT" || fit == "NoiseFFT") && !(m_ChBase->WfmBase::GetDoFFT()))
             {
...
TH1D* DUTChannel::GetFit(std::string fit, int i)
{
    ...
    else if (fit == "SigFFT" && m_ChBase->WfmBase::GetDoFFT()) return (TH1D*)m_ChSignalFFTFt->Clone();
    else if (fit == "NoiseFFT" && m_ChBase->WfmBase::GetDoFFT()) return (TH1D*)m_ChNoiseFFTFt->Clone();
...
// --------------------------------------------------------------------------------------------------------------
void DUTChannel::ChannelInit(WfmBase *tBase)
{
    ...
    m_ChChargeCut = -99.;
    m_ChBinSz = -99;
    m_ChLowLim = std::make_pair(-99, -99);
    m_ChUpLim = std::make_pair(-99, -99);
...
{
    m_ChWaveform->WaveForm::InitializeWaveForm(1);
    m_ChWaveform->WfmBase::SetVerbose(m_ChBase->GetVerbosity());
...
bool DUTChannel::AppendWaveform(WaveForm* k)
{
    ...
    if ((k->WaveForm::GetVoltage())->size() != 0 && (k->WaveForm::GetTime())->size() != 0)
       {           
        ...
        // Start populating the vectors of the Channel
    ...
    for (unsigned int k = 0; k < (apoints-1); k++)
        {
         ...
         if (m_ChBase->WfmBase::GetFitMethode() == "rootInt") m_MeanChPulse.at(k) = CalculateMeanGLandau(&pointvolt, &m_ChComplete, m_MeanChPlFt[k], m_MeanChPlFtChi2[k], 3, "root", true);
         else m_MeanChPulse.at(k) = CalculateMeanGLandau(&pointvolt, &m_ChComplete, m_MeanChPlFt[k], m_MeanChPlFtChi2[k], 3, m_ChBase->WfmBase::GetFitMethode(), true);
         m_ChBase->WfmBase::ProgressBar(k, (apoints-1));
         pointvolt.clear();
        }
...
int DUTChannel::updateChProperties(bool waveshape, TTree* wavetree)
{
  ...
  if (waveshape) CalculateShape(wavetree);
        ...
        m_ChMeanPolarity = round(m_ChBase->WfmBase::Mean(&ChPol));
       }
  ...
  m_ChFitMaxIndx = CalculateMeanG(&ChMaxIndx, &m_ChComplete, m_ChFitMaxIndxFt, m_ChFitMaxIndxChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitMaxVolt = CalculateMeanGLandau(&m_ChMaxVolt, &m_ChComplete, m_ChFitMaxVoltFt, m_ChFitMaxVoltChi2, 2, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  m_ChFitMinIndx = CalculateMeanG(&ChMinIndx, &m_ChComplete, m_ChFitMinIndxFt, m_ChFitMinIndxChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitMinVolt = CalculateMeanG(&m_ChMinVolt, &m_ChComplete, m_ChFitMinVoltFt, m_ChFitMinVoltChi2, 2, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  m_ChFitStrIndx = CalculateMeanG(&ChStrIndx, &m_ChComplete, m_ChFitStrIndxFt, m_ChFitStrIndxChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitEndIndx = CalculateMeanG(&ChEndIndx, &m_ChComplete, m_ChFitEndIndxFt, m_ChFitEndIndxChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  if (fabs(m_ChFitNoiseSlope.first) < 0.1)
     {
      ...
      m_ChFitNoiseErr = CalculateMeanG(&m_ChNoiseErr, &m_ChComplete, m_ChNoiseErrFt, m_ChNoiseErrChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
      ...
      m_ChFitNoise = CalculateMeanG(&m_ChNoise, &m_ChComplete, m_ChNoiseFt, m_ChNoiseChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
      if (m_ChNoiseChi2 != -1) 
         {
        ...
        m_ChFitNoiseErr = CalculateMeanGLinear(&m_ChNoiseErr, &m_ChComplete, m_ChNoiseErrFt, m_ChNoiseErrChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
        if (m_ChNoiseErrChi2 != -1) 
           {
            m_ChNoiseErrFt->SetNameTitle(Form("Ch_NoiseErr%02u", m_channelID), Form("Noise uncertainty, Channel %02u;Noise uncertenty [V];Entries", m_channelID));
            m_ChNoiseErrFt->SetDirectory(nullptr); // detach histo from open directory
           }
        m_ChFitNoise = CalculateMeanGLinear(&m_ChNoise, &m_ChComplete, m_ChNoiseFt, m_ChNoiseChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
        if (m_ChNoiseChi2 != -1) 
           {
            m_ChNoiseFt->SetNameTitle(Form("Ch_Noise%02u", m_channelID), Form("Noise, Channel %02u;Noise [V];Entries", m_channelID));
            m_ChNoiseFt->SetDirectory(nullptr); // detach histo from open directory
  ...
  if (fabs(m_ChFitPedestalSlope.first) < 0.1)
     {
      ...
      m_ChFitPedestal = CalculateMeanG(&m_ChPedestal, &m_ChComplete, m_ChPedestalFt, m_ChPedestalChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
      ...
      m_ChFitPedestalErr = CalculateMeanG(&m_ChPedestalErr, &m_ChComplete, m_ChPedestalErFt, m_ChPedestalErChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
      if (m_ChPedestalErChi2 != -1) 
        ...
        std::cout << __FUNCTION__ << " INFO : Significant pedestal slope detected ("<< fabs(m_ChFitNoiseSlope.first)
                                  <<"), applying Linear x Gaussian fit" << std::endl;
        m_ChFitPedestal = CalculateMeanGLinear(&m_ChPedestal, &m_ChComplete, m_ChPedestalFt, m_ChPedestalChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
        if (m_ChPedestalChi2 != -1) 
           {
            m_ChPedestalFt->SetNameTitle(Form("Ch_Pedest%02u", m_channelID), Form("Pedestal, Channel %02u;Baseline [V];Entries", m_channelID));
            m_ChPedestalFt->SetDirectory(nullptr); // detach histo from open directory
           }
        m_ChFitPedestalErr = CalculateMeanGLinear(&m_ChPedestalErr, &m_ChComplete, m_ChPedestalErFt, m_ChPedestalErChi2, 1, m_ChBase->WfmBase::GetFitMethode(), false);
        if (m_ChPedestalErChi2 != -1) 
           {
            m_ChPedestalErFt->SetNameTitle(Form("Ch_PedestErr%02u", m_channelID), Form("Pedestal Uncertainty, Channel %02u;Baseline uncertainty [V];Entries", m_channelID));
            m_ChPedestalErFt->SetDirectory(nullptr); // detach histo from open directory
  ...
  m_ChFitMaxTime = CalculateMeanG(&m_ChMaxTime, &m_ChComplete, m_ChMaxTimeFt, m_ChMaxTimeChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitMinTime = CalculateMeanG(&m_ChMinTime, &m_ChComplete, m_ChMinTimeFt, m_ChMinTimeChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitCharge = CalculateMeanGLandau(&m_ChCharge, &m_ChComplete, m_ChChargeFt, m_ChChargeChi2, 2, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  if (m_ChChargeChi2 != -1) 
     {
      m_ChChargeFt->SetNameTitle(Form("Ch_Charge%02u", m_channelID), Form("Collected Charge, Channel %02u;Charge [Q];Entries", m_channelID));
  ...
  m_ChFitRiseTime = CalculateMeanG(&m_ChRiseTime, &m_ChComplete, m_ChFitRiseTimeFt, m_ChFitRiseTimeChi2, 2, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  for (unsigned int i = 0; i < 19; i++) 
      {
       ...
       m_ChBase->WfmBase::ProgressBar(i, 19);
       m_FitChCFDTime.push_back(CalculateMeanG(&m_ChCFDTime[i], &m_ChComplete, m_ChCFDTimeFt[i], m_ChCFDTimeChi2[i], 2, m_ChBase->WfmBase::GetFitMethode(), false));
       ...
       m_FitChDVDTCFD.push_back(CalculateMeanGLandau(&m_ChDVDTCFD[i], &m_ChComplete, m_ChDVDTCFDFt[i], m_ChDVDTCFDChi2[i], 3, m_ChBase->WfmBase::GetFitMethode(), true));
       ...
       m_FitChCFDToT.push_back(CalculateMeanG(&m_ChCFDToT[i], &m_ChComplete, m_ChCFDToTFt[i], m_ChCFDToTChi2[i], 4, m_ChBase->WfmBase::GetFitMethode(), false));
  ...
  m_ChFitTriggTime = CalculateMeanG(&m_ChTriggTime, &m_ChComplete, m_ChTriggTimeFt, m_ChTriggTimeChi2, 3, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  m_ChFitDVDTMax = CalculateMeanGLandau(&m_ChDVDTMax, &m_ChComplete, m_ChDVDTMaxFt, m_ChDVDTMaxChi2, 2, m_ChBase->WfmBase::GetFitMethode(), true);
  ...
  m_ChFitTriggToT = CalculateMeanGLandau(&m_ChTriggToT, &m_ChComplete, m_ChTriggToTFt, m_ChTriggToTChi2, 5, m_ChBase->WfmBase::GetFitMethode(), false);
  ...
  if (m_ChBase->WfmBase::GetDoFFT())
     {
      m_ChFitSignalFFT = CalculateMeanG(&m_ChSignalFFT, &m_ChComplete, m_ChSignalFFTFt, m_ChSignalFFTChi2, 2, m_ChBase->WfmBase::GetFitMethode() + "VarBin", true);
      ...
      m_ChFitNoiseFFT = CalculateMeanG(&m_ChNoiseFFT, &m_ChComplete, m_ChNoiseFFTFt, m_ChNoiseFFTChi2, 2, m_ChBase->WfmBase::GetFitMethode() + "VarBin", true);
  ...
  m_ChFitIsSignal.first = m_ChBase->WfmBase::Mean(&m_ChIsSignal);
  m_ChFitIsSignal.second = m_ChBase->WfmBase::BayesianErr(&m_ChIsSignal, true);
  ...
  m_ChFitIsInWindow.first = m_ChBase->WfmBase::Mean(&m_ChIsInWindow);
  m_ChFitIsInWindow.second = m_ChBase->WfmBase::BayesianErr(&m_ChIsInWindow, true);
  ...
  m_ChFitVoltStr.first = m_ChBase->WfmBase::Mean(&m_ChVoltStr);
  m_ChFitVoltStr.second = m_ChBase->WfmBase::BayesianErr(&m_ChVoltStr, true);
  ...
  m_ChFitComplete.first = (m_ChBase->WfmBase::Mean(&m_ChComplete)) / 6;
  m_ChFitComplete.second = m_ChBase->WfmBase::BayesianErr(&m_ChComplete, 6);
                                                            ...
                                                            std::string fitopt, bool discrt)
{
    ...
    if (as == 0) std::cout << __FUNCTION__ << " EROOR : Cannot perform Gaussian x Landau fit on empty vector!!" << std::endl;
          ...
          if (fitopt == "RooFit") qual = m_ChBase->WfmBase::RooConvFit(&volt, VoltMPV, VoltSigma, "LanXGau");
          else if (fitopt == "rootInt") qual = m_ChBase->WfmBase::IterativeFit(&volt, VoltMPV, VoltSigma, hitHist, fitChi2, "LandauXGaussInt", std::make_pair(-1, -1), discrt);
          else if (fitopt == "rootVarBin") qual = m_ChBase->WfmBase::IterativeFit(&volt, VoltMPV, VoltSigma, hitHist, fitChi2, "LandauXGaussVarBin", std::make_pair(-1, -1), discrt);
          else if (fitopt == "rootIntVarBin") qual = m_ChBase->WfmBase::IterativeFit(&volt, VoltMPV, VoltSigma, hitHist, fitChi2, "LandauXGaussIntVarBin", std::make_pair(-1, -1), discrt);
          else if (fitopt == "root") qual = m_ChBase->WfmBase::IterativeFit(&volt, VoltMPV, VoltSigma, hitHist, fitChi2, "LandauXGauss", std::make_pair(-1, -1), discrt);
                                                     ...
                                                     std::string fitopt, bool discrt)
{
    ...
    if (evnt == 0) std::cout << __FUNCTION__ << " EROOR : Cannot perform Gaussian fit on empty vector!!" << std::endl;
          ...
          if (fitopt == "rootInt") qual = m_ChBase->WfmBase::IterativeFit(&real, mean, sigma, hitHist, fitChi2, "GaussInt", std::make_pair(-1, -1), discrt);
          else if (fitopt == "rootVarBin") qual = m_ChBase->WfmBase::IterativeFit(&real, mean, sigma, hitHist, fitChi2, "GaussVarBin", std::make_pair(-1, -1), discrt);
          else if (fitopt == "rootIntVarBin") qual = m_ChBase->WfmBase::IterativeFit(&real, mean, sigma, hitHist, fitChi2, "GaussIntVarBin", std::make_pair(-1, -1), discrt);
          else if (fitopt == "root") qual = m_ChBase->WfmBase::IterativeFit(&real, mean, sigma, hitHist, fitChi2, "Gauss", std::make_pair(-1, -1), discrt);
          if (qual == 0) mean.second = sigma.first;
          else {
                mean.first = m_ChBase->WfmBase::Mean(&real);
                mean.second = m_ChBase->WfmBase::Stdev(&real);
...
std::pair <double, double> DUTChannel::CalculateMeanLinear(std::vector<double>* vec, std::vector<bool>* signal, std::vector<double>* vecErr)
{
    ...
    if (evnt == 0) std::cout << __FUNCTION__ << " EROOR : Cannot perform Linear fit on empty vector!!" << std::endl;
          ...
          int res = -99;
          if (vecErr != NULL) res = m_ChBase->WfmBase::LinearFit(&real, slope, intercept, &realErr);
          else res = m_ChBase->WfmBase::LinearFit(&real, slope, intercept);
          if (res != 0) 
             {
              std::vector <double> deriv = m_ChBase->WfmBase::Derivate(&real);
              slope.first = m_ChBase->WfmBase::Mean(&deriv);
              slope.second = m_ChBase->WfmBase::Stdev(&deriv);
                                                           ...
                                                           int level, std::string fitopt, bool discrt)
{
    ...
    if (evnt == 0) std::cout << __FUNCTION__ << " EROOR : Cannot perform Linear fit on empty vector!!" << std::endl;
          ...
          real.reserve(evnt);
          for (unsigned int k = 0; k < evnt; k++) if (signal->at(k) > level && vec->at(k) != -1.) real.push_back(vec->at(k));
          int res = m_ChBase->WfmBase::RooConvFit(&real, mean, sigma, "LinXGau") == 0;
          if (res != 0) 
             {
